...
def removeFatlikeTissue(s, params):
    ...
    img_reduced = morphology.remove_small_holes(s["img_mask_use"], fat_cell_size)
    img_small = img_reduced & np.invert(s["img_mask_use"])
    img_small = ~morphology.remove_small_holes(~img_small, 9)
...
def fillSmallHoles(s, params):
    ...
    min_size = int(params.get("min_size", 64))
    img_reduced = morphology.remove_small_holes(s["img_mask_use"], min_size)
    img_small = img_reduced & np.invert(s["img_mask_use"])
